...
class Asset_Rename_Exporter(object):
    ...
    def __init__(self):
        ...
        if self.platform == "nt":
            self.photoshop = r"C:\Program Files\Adobe\Adobe Photoshop CC 2014\Photoshop.exe"
    ...
    def makePngFiles(self,filetoconvert,newFileName):
        workDir = os.path.split(filetoconvert)[0]

        current_script_path = os.path.dirname(os.path.realpath(__file__))
        javascript_file_export = os.path.realpath(os.path.join(current_script_path,'..','photoshop_scripts','fixed_files.jsx'))
        psd_export_folder = os.path.realpath(os.path.join(os.path.split(filetoconvert)[0],'..','Exported'))
        
        location_of_temp = self._make_temp_folder(psd_export_folder)
        filestoconvert = os.path.join(workDir,filetoconvert)
        
        #sends for conversion
        self._windows_jsx_file_prep(filestoconvert, javascript_file_export, location_of_temp, newFileName,False,self.photoshop) 
    ...
    def makeScreenShots(self, files_to_convert,seperator):
        ...
        exportFolder = str(os.path.join(seperator,"Jpegs"))
        exportFolder = exportFolder.replace("{", "")
        location_of_temp = self._make_temp_folder(exportFolder)
        
        for file_to_convert in files_to_convert:

            file_to_convert = os.path.join(seperator,file_to_convert)
            ...
            new_file_name = os.path.join(exportFolder,fileName)
    ...
    def makeRawFilesTiff(self,files_to_convert,seperator):
        #relative path of all script files and put out the art to new locations
        current_script_path = os.path.dirname(os.path.realpath(__file__))
        tiff_file_export = os.path.realpath(os.path.join(current_script_path,'..','photoshop_scripts','photoshop_tiffSave_BATCH.jsx'))
        ...
        for file_to_convert in files_to_convert:
            ...
            splitFileName = os.path.split(file_to_convert)[-1]
            fileName = splitFileName.split('.')[0] + ".tiff"
            ...
            if not(os.path.exists(new_file_name)):
            #sends for conversion
                self._windows_jsx_file_prep(file_to_convert, tiff_file_export, location_of_temp, new_file_name,False,self.photoshop)
