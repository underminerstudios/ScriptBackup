...
class UserInterface:
    def __init__(self, master):
        ...
        self.makepngsButton = Button(frame,
                          text="Make pngs",
                          command=self.makePngs)
        self.makepngsButton.pack(side=LEFT)
        self.convertArtButton = Button(frame,
                          text="Convert file To Tiff",
                          command=self.maketiffs)
        self.convertArtButton.pack(side=LEFT)
        


    def photoshop_renamer_and_export(self):
        self.run_everything_art(False,False)
      
    def makePngs(self):
        self.run_everything_art(False,True)
    ...
    def maketiffs(self):
        asset_to_convert = self.button('Select The Raw File(s) that you want turned into Tiffs', [('PSD', '*.psd')], "Seems like something happened. Please Try again!", 1)
        ...
        asset_rename.makeRawFilesTiff(string_of_files,seperator)
        root.destroy()

    def run_everything_art(self,do_both,old):
        ...
        for file_name in assets_to_rename:
            ...
            try:
                new_file_name = new_file_name.replace("WBY_","")
                new_file_name = new_file_name.replace("_optimized", "")
            except:
                pass
        ...
        for file_to_convert in files_to_convert:
            if not(old == True):
                ...
                asset_rename.makePngFiles(os.path.join(folder_name,"WBY_" + file_to_convert + "_optimized" + ".psd"), files_to_convert[file_to_convert])
