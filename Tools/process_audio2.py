...
for root, dirs, files in os.walk(os.path.realpath(os.path.join("..","Raw","Utterances","uncut"))):
    for dirname in dirs:
        if os.path.isdir(dirname) and re.match(r"\d\d_\d\d", os.path.basename(dirname)):
            for src in glob.glob(os.path.join(dirname,"*.wav")):
                if re.search(r"\d\d\d_\d\d\d_\d\d\d.wav$", src):
                    shot_id, ext = os.path.splitext(os.path.basename(src))
                    dest = os.path.join("..","Assets","Sounds","Utterances") + shot_id + "_ALL.ogg"
                    print dest
...
# process music
for ifn in glob.glob(os.path.join("..","Raw","Music","*.m4a")):
  stem, ext = os.path.splitext(os.path.basename(ifn))
  ofn = os.path.join("..","Assets","Sounds","Music") + stem + ".ogg"
