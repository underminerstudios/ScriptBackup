...
glm = GLMax()
enm = EnlightenClass()
toolpath = "S:\\code\\python\\tools\\Projects\\_Asphalt\\A9Enlighten\\"
			...
			if obj.parent != None:
				...
				if oParent[0] == "inst":
					...
					if glm.classOf(mat) == "Standardmaterial":
							...
							if submat.selfIllumMap != None:
								matDict['TextureE'] = (submat.selfIllumMap ).filename
							...
							if matDict['TextureE'] != None:
								eEmissive = ET.SubElement(eMaterial, "emissive")
								eTextureE = ET.SubElement(eEmissive, "texture")
								eTextureE.text = matDict['TextureE']
...
#Setup Combo List
	...
	def connect(self):
		...
		self.ui_btn_processall.clicked.connect(self.ProcessAll)
...
#			shutil.copy("S:\\code\\python\\tools\\Projects\\_Asphalt\\A9Enlighten\\Bin\\georadiosity.bat",(str(scenedir) + "\\GeoMaxExport\\"))
	...
	def ProcessAll(self):
		ckPrecompute = self.ui_ck_precompute.checkState()
		ckPrecompute = checkboxtobool(ckPrecompute)
		scenedir = self.ui_ln_scenepath.text()
		enm.AssignA9MeshParams()
		for obj in mxs.objects:
			objname = (obj.name).split('_')
	...
	def ExecPrecomp(self):
		ckPrecompute = self.ui_ck_precompute.checkState()
		ckPrecompute = checkboxtobool(ckPrecompute)
		scenedir = self.ui_ln_scenepath.text()
		if ckPrecompute == 'true':
			self.WriteLuaExec()
			precompcmd = ('S:\\code\\python\\tools\\Projects\\_Asphalt\\A9Enlighten\\Bin\\GeoRadiosity.exe -scene:' + str(scenedir) + '\\GeoMaxExport\\GeoMaxExport.scene -script:' + str(scenedir) + '\\enAutoExec.lua')
			QProcess.startDetached(precompcmd)
#			precompcmd = ('S:\\code\\python\\tools\\Projects\\_Asphalt\\A9Enlighten\\Bin\\GeoBuildMonitor.exe /t:Precompute /p:InputScene=' + str(scenedir) + '\\GeoMaxExport\\GeoMaxExport.scene')
#			QProcess.startDetached(precompcmd)
	...
	def ConvertTex(self):
		...
		if ckExportAll == 2:
			...
			for filename in os.listdir(iPath):
				converttotexcmd = ("S:\\code\\python\\tools\\Projects\\_Asphalt\\A9Enlighten\\Bin\\GeoConvert.exe /ConvertTexToExr ")
	...
	def WriteLuaExec(self):
		scenedir = self.ui_ln_scenepath.text()
		scenevar = ('local sScene = ("' + str(scenedir) + '\\GeoMaxExport\\GeoMaxExport.scene")\n')
		lightsvar = ('local sLights = ("' + str(scenedir) + '\\GeoMaxExport\\GeoMaxExport.lights")\n')
		scenecmd = 'app.LoadScene( sScene ) ;\n'
		lightspath = (str(scenedir)  + '\\GeoMaxExport\\GeoMaxExport.lights')
		lightscmd = 'app.LoadLightsImmediate( sLights ) ;\n'
		
		
		precompcmd = 'app.StartPrecompute( )\n'
		exitcmd = 'app.ExitApplication( )'
		enAutoExec = open((str(scenedir) + '\\enAutoExec.lua'), 'w')
		enAutoExec.write(scenevar)
		enAutoExec.write(scenecmd)
#		if os.path.isfile(lightspath):
#			enAutoExec.write(lightsvar)
#			enAutoExec.write(lightscmd)
		enAutoExec.write(precompcmd)
		enAutoExec.write(exitcmd)
		enAutoExec.close()
	...
	def WriteBatFile(self):
		...
		ln1 = '@echo off\n'
		ln2 = 'cd /d %~dp0\n'
		ln3 = 'start S:\\code\\python\\tools\\Projects\\_Dungeon_Hunter\\Enlighten\\Bin\\GeoRadiosity.exe -scene:' + scene
	...
	def ExportMaxScene(self):
		...
		if os.path.isdir(str(scenedir)):
			shutil.copy("S:\\code\\python\\tools\\Projects\\_Asphalt\\A9Enlighten\\GeoMaxExport_default.bp", (str(scenedir)))
