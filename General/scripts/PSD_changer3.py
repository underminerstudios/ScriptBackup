...
exportTypes = {'diff':'_DIF', 'nrm':'_NRM', 'spec':'_SPC', 'trans':'_TRANS', 'ao':'_AO', 'bmp':'_BMP', 'epidermal':'_EPIDERMAL', 
'subdermal':'_SUBDERMAL', 'back_scatter':'_BACKSCATTER', 'ref':'_REF', 'glow':'_GLOW', 'diff_level':'_DIFF_LEVEL'}
...
while wantACopy:
   wantACopy = False
   copy = raw_input('Do you want a copy in the origonal locations as well? \n 1 for "yes" 0 for "no"\n')
   if copy == '1':
      everythingExport = '1'
      psdOut = '0'
      pass
...
# Warns people to wait because photoshop is loading... which TAKES FOREVER at time.
         ...
         for layerSet in layerSets:
            ...
            if (lsName in exportTypes):
               ...
               if psdOut == '1' :
                  ...
                  finishedCopyFile = os.path.join(psOutLocation , os.path.splitext(copyFile)[0] + exportTypes[lsName] + extension.lower())
               if everythingExport == '1':
                  copyFile = os.path.basename(psdFile)
                  finishedCopyFile = os.path.join(psOutLocation , os.path.splitext(copyFile)[0] + exportTypes[lsName] + extension.lower())
                  finishedFile = os.path.splitext(psdFile)[0] + exportTypes[lsName] + extension.lower()
               else :
                  finishedFile = os.path.splitext(psdFile)[0] + exportTypes[lsName] + extension.lower()
               ...
               if (os.path.exists(finishedCopyFile)):
                  psdTime = os.stat(psdFile)[8]
                  copyTime = os.stat(finishedCopyFile)[8]
                  

                  if (psdTime > exportTime):
                     os.remove(finishedFile)
                  if (psdTime > copyTime):
                     os.remove(finishedCopyFile)
                  else :
                     print 'Can not process files of the same name!'
                # Export files for this layer Group
               ...
               if copy == '1':
                  if (not os.path.exists(finishedCopyFile)):
                     doc = psApp.Open(psdFile)
                     doc.SaveAs(finishedCopyFile, options, True)
                     print 'exporting:', finishedCopyFile
               else:
